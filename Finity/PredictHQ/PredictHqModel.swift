...
class PredictHqModel: ObservableObject {
    ...
    private var predictHQ_API = PredictHQ_API()
    private var dbManager = FirestoreManager()
    public var postUser: User?
    @Published var isFetching = true
    
    public func fetchUser(userId: String) {
        dbManager.fetchTheseUsers(userIds: [userId]) { users in
    ...
    public func fetchResonatingArgumentStored(sentence: String, handler: @escaping(_ argument: String, _ score: Double) -> Void) {
        var sentenceList = [String]()
        
        // Removing stop words
        let stopWords = getStopWords()
        sentence.byWordsAndRanges.forEach { word, _ in
            if !stopWords.contains(word) {
                sentenceList.append(word)
            }
        }
        
        // fetching arguments similar to use words
        var mostResonating = ""
        var mostResonatingScore: Double = 0
        let sentenceVector = vector(for: sentence)
        var index = 0
        if sentenceList.count > 0 {
            predictHQ_API.fetchData(request_param: "q=\(sentenceList[index])&sort=relevance") { results in
                results.forEach { result in
                    let title = result["title"] as! String
                    let description = result["description"] as! String
                    let titleVector = self.vector(for: title)
                    let descriptionVector = self.vector(for: description)
                    if self.cosineSimilarity(a: sentenceVector, b: titleVector) > mostResonatingScore {
                        mostResonating = title
                        mostResonatingScore = self.cosineSimilarity(a: sentenceVector, b: titleVector)
                    }
                    if self.cosineSimilarity(a: sentenceVector, b: descriptionVector) > mostResonatingScore {
                        mostResonating = description
                        mostResonatingScore = self.cosineSimilarity(a: sentenceVector, b: descriptionVector)
                    }
                }
                handler(mostResonating, mostResonatingScore)
            }
        } else {
            handler(mostResonating, 100)
        }
    }
}
...
extension PredictHqModel {
    private func vector(for string: String) -> [Double] {
    ...
    private func getStopWords() -> [String] {
